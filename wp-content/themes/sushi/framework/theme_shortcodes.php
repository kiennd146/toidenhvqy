						...
						$out .= '<div class="entry-thumb">';
							...
							elseif( $format === "gallery" ):
							  ...
							  if( array_key_exists("items", $post_meta) ):
								  ...
								  foreach ( $post_meta['items'] as $item ) { $out .= "<li><img src='{$item}' /></li>";	}
								  $out .= "</ul>";
							  endif;
							elseif( $format === "video" ):
								  $post_meta =  get_post_meta($post->ID ,'_dt_post_settings', true);
						...
						if($show_meta != "false"):
							$out .= '<div class="post-meta">';
								$out .= '<ul>';								
									...
									$out .= '<li class="cat">'.$thiscats.'</li>';
									}
									$out .= get_the_tag_list('<li class="tags">', ', ', '</li>');
								$out .= '</ul>';
							$out .= '</div>';
						endif;
...
if(!function_exists('dt_gallery_items')) {
	...
	function dt_gallery_items( $atts, $content = null ) {
		extract(shortcode_atts(array(
			'limit' => -1,
			'categories' => '',
			'posts_column' => 'one-half-column', // one-column, one-third-column, one-fourth-column
			'filter' => ''
		), $atts));
		...
		if($wp_query->have_posts()): 
		 ...
		 $out .= '<div class="gallery-container">';
			while($wp_query->have_posts()): $wp_query->the_post(); 
				$terms = wp_get_post_terms($post->ID, 'gallery_entries', array("fields" => "names")); array_walk($terms, "arr_strfun");
				
				$out .= '<div class="'.$li_class." ".strtolower(implode(" ", $terms)).'">';
...
if(!function_exists('dt_theme_service')) {
	...
	function dt_theme_service($atts, $content = null) {
		...
		$out .= '<div class="dt-service">';
			if($animation != NULL) $out .= '<i class="fa '.$icon.' animate" data-animation="'.$animation.'" data-delay="'.$delay.'"></i>';	
			else $out .= '<i class="fa '.$icon.'"></i>';	
...
#THEME CUSTOM SERVICE SHORTCODE...
if(!function_exists('dt_theme_custom_service')) {

	function dt_theme_custom_service($atts, $content = null) {
		extract(shortcode_atts(array(
			'animation' => '',								 
			'delay' => '300',
			'icon' => 'icon-burger',
		...
		$out .= '<div class="dt-custom-service">';
	...
	add_shortcode('dt_theme_custom_service', 'dt_theme_custom_service');
}
...
#CATALOG CATEGORIES...
if(!function_exists('dt_catalog_cats')) {
	
	function dt_catalog_cats($atts, $content = null) {
		extract(shortcode_atts(array(
			'cat_ids' => ''
		), $atts));

		if($cat_ids == '') {
			$cat_ids = get_terms( array('catalog_entries'), array('fields' => 'ids'));
		}
		else {
			$cat_ids = explode(',', $cat_ids);
		}
		
		$out = '<ul class="dt-sc-offer-list">';
		
            foreach($cat_ids as $term) {
                $myterm = get_term_by('id', trim($term), 'catalog_entries');
				$ic = get_option("taxonomy_term_$term");
				
				$out .= '<li><span class="fa '.$ic["icon_class"].'"></span>'.$myterm->name.' - '.$myterm->description.'</li>';
            }
	...
	add_shortcode('dt_catalog_cats', 'dt_catalog_cats');
}

#CATALOG ITEMS...
if(!function_exists('dt_catalog_items')) {
	
	function dt_catalog_items($atts, $content = null) {
		extract(shortcode_atts(array(
			'item_ids' => '',
		...
		if($wp_query->have_posts()):
			...
			while($wp_query->have_posts()): $wp_query->the_post();
				...
				$dprice = isset( $item_meta['dummyprice']) ? $item_meta['dummyprice'] : NULL;
				$price = isset( $item_meta['price']) ? $item_meta['price'] : NULL;
				
				$out .= '<li>';
					$out .= '<span class="item-name">'.get_the_title().'</span>';
					$out .= '<span class="item-price">';
					
					if($show_dummy_price == 'true') $out .= '<del>'.$dprice.'</del>';
	...
	add_shortcode('dt_catalog_items', 'dt_catalog_items');
}

#CATALOG ITEMS TYPE2...
if(!function_exists('dt_catalog_items_type2')) {
	
	function dt_catalog_items_type2($atts, $content = null) {
		extract(shortcode_atts(array(
			'item_ids' => '',
			'column' => '3'
		), $atts));
		...
		{
			case 2:
				$col_class = "dt-sc-one-half no-margin column";
				break;

			case 3:
				$col_class = "dt-sc-one-third no-margin column";
				break;
		...
		if($wp_query->have_posts()):
			...
			$out .= '<div class="'.$col_class.'">';
				$out .= '<div class="catalog-menu">';
					$out .= '<span class="rounded">';
						$out .= '<a href="javascript:void(0)">'.get_the_post_thumbnail(get_the_ID(), 'catalog-twocolumn', array('title' => get_the_title())).'</a>';
					$out .= '</span>';
					$out .= '<h5><a href="javascript:void(0)">'.get_the_title().'</a></h5>';
					$out .= '<a class="dt-sc-button small" href="javascript:void(0)">'.$price.'</a>';
				$out .= '</div>';
			$out .= '</div>';
	...
	add_shortcode('dt_catalog_items_type2', 'dt_catalog_items_type2');
}
