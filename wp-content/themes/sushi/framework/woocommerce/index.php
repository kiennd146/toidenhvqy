...
function dt_woocommerce_template_loop_product_thumbnail() {
	...
	$image = !empty( $image ) ? $image : "<img src='http://placehold.it/520x520' />";	
	
	$attachment_ids = $product->get_gallery_attachment_ids();
	$secondary_image_id = $attachment_ids['0'];

	$image1 = wp_get_attachment_image( $secondary_image_id, array(1000,1000), '', $attr = array( 'class' => 'secondary-image attachment-shop-catalog' ) );
...
function dt_woocommerce_shop_overview_extra_div_close() {
	...
	$out .= $add_to_cart;
	if(get_post_meta($product->id, 'cus_pro_type', true) != NULL) {
		$value = get_post_meta($product->id, 'cus_pro_type', true);
...
function dt_woocommerce_show_product_sale_flash() {
	...
	if(get_post_meta($product->id, 'cus_pro_type', true) != "") {
		$value = get_post_meta($product->id, 'cus_pro_type', true);
		$out .= '<span class="'.$value.'">'.$value.'</span>';
	}
...
#VEG, NON-VEG CUSTOM FIELDS...
add_action( 'woocommerce_product_options_pricing', 'wc_type_product_field' );
function wc_type_product_field() {
	global $post; ?>
    
    <p class="form-field product_field_type">
    <label for="product_field_type"><?php _e( 'Veg / Non-Veg', 'iamd_text_domain' ); ?></label>
    <?php $product_cus_type = get_post_meta( $post->ID, 'cus_pro_type', true ); ?>
    
    <select id="product_field_type" name="_product_cus_type"><?php
		echo '<option value="">'.__( 'Choose Type', 'iamd_text_domain' ).'</option>';
		echo '<option value="cls-veg" '.selected( 'cls-veg', $product_cus_type, true, false).'>'.__( 'Vegeterian', 'iamd_text_domain' ).'</option>';
		echo '<option value="cls-non-veg" '.selected( 'cls-non-veg', $product_cus_type, true, false).'>'.__( 'Non-Vegetarian', 'iamd_text_domain' ).'</option>'; ?>
    </select>
    </p><?php
}

#CUSTOM FIELD SAVE OPTION...
add_action( 'save_post', 'wc_cus_type_save_product');
function wc_cus_type_save_product( $product_id ) {
	
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE )
	return;
	
	if ( isset( $_POST['_product_cus_type'] ) )
		update_post_meta( $product_id, 'cus_pro_type', $_POST['_product_cus_type'] );
	else
		delete_post_meta( $product_id, 'cus_pro_type' );
} ?>